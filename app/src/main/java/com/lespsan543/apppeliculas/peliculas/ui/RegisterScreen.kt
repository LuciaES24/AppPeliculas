...
import com.lespsan543.apppeliculas.peliculas.data.util.Constants
    ...
    BoxWithConstraints {
            ...
            horizontalAlignment = Alignment.CenterHorizontally)
        {
            Text(text = "Registrarse", fontSize = 30.sp,
                modifier = Modifier.padding(top = height*0.1f), color = Color.White, fontFamily = Constants.FONT_FAMILY
            )
            Spacer(modifier = Modifier.height(height * 0.08f))
            TextField(value = name,
                onValueChange = { viewModel.writeName(it) },
                label = { Text(text = "Name...", color = Color.DarkGray, fontFamily = Constants.FONT_FAMILY) },
            ...
            TextField(value = email,
                ...
                label = { Text(text = "Email...", color = Color.DarkGray, fontFamily = Constants.FONT_FAMILY) },
                keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.Email),
                colors = ExposedDropdownMenuDefaults.textFieldColors(
                    containerColor = Color.White,
                    textColor = Color.Black
            ...
            TextField(value = password,
                ...
                label = { Text(text = "Password...",  color = Color.DarkGray, fontFamily = Constants.FONT_FAMILY) },
                colors = ExposedDropdownMenuDefaults.textFieldColors(
                    containerColor = Color.White,
                    textColor = Color.Black,
            ...
            {
                Text(text = "Registrarse", fontFamily = Constants.FONT_FAMILY)
            }
            Spacer(modifier = Modifier.height(height * 0.07f))
            Row {
                Text(text = "¿Ya estas registrado?", color = Color.White, fontFamily = Constants.FONT_FAMILY)
            ...
            if (wrong == true){
                AlertDialog(onDismissRequest = {  },
                    confirmButton = { Button(onClick = { viewModel.closeDialog() }) {
                        Text(text = "Aceptar", fontFamily = Constants.FONT_FAMILY)
                    }
                    },
                    text = { Text(text = "Algún dato es incorrecto", fontFamily = Constants.FONT_FAMILY)}
                )
            }
        }
    }
